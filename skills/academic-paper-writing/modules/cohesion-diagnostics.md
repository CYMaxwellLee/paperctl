...
> ⛔⛔ **先跑 `structure-first.md` 的 §0 結構盤點，否則六問無效。**
> 六問只檢查**段落內部的接縫**，它假設「文字已經在那裡、內容也該在那裡」。改寫學生初版時
> 直接跑六問，產出就是「**學生骨架 ＋ 修好的字**」—— 六問越熟練越容易停在 line-edit。
> **這不是六問寫得不夠細，是層級不對：該不該存在、該講什麼，是上一層的事。**
> （主人 2026-07-29：「很多時候會被學生寫作的初版本帶走，變成只在改一些小文字錯誤」）
>
...
- **⛔ 前提：`structure-first.md` 的功能段清單已經寫好、對照過了**
- 改寫完一段之後
- 整節 line-edit 的時候
...
> ⚠️ 口語通稱「六問」。那是這套診斷的**名字，不是條數** —— 目前八條，每加一條就更新這裡。
> （原標題寫「六個」而底下實際七條，2026-07-30 補第八條時一併修掉。）
...
### 8. 沒有連接詞的地方問「這兩句能不能對調」

**觸發**：一段裡連續幾句話，每句換一個主詞、各講一件事，句與句之間沒有任何連接。
前七問全部**由觸發字定位**，唯獨這一問要抓的是**沒有觸發字的地方** —— 所以它靠讀，不靠掃。

**檢查：把相鄰兩句對調。** 讀起來沒有變壞，就是在列 bullets 而不是在論述。
有 cohesion 的兩句對調之後一定會壞，因為後句用到的東西前句還沒給。

**修法：given-new** —— 每句**句首放上一句已經給過的東西，句尾才放新資訊**；下一句再從那個新資訊接下去。
修完對調測試會自動失敗，那正是通過。

⚠️ 這一問**不是新規則**，是既有自查的展開版。`style-guide.md` 第五節早就寫著
「把邏輯連接詞遮住，剩下的句子若都能獨立存在，這段是在列舉而非論述」——
那條給的是**診斷**（告訴你在列舉），本問補上**修法**（given-new）與**驗收**（對調測試）。

⛔ 別跟第 1 問混淆：第 1 問問「**有**連接詞時，兩邊軸對不對齊」；本問問「**沒有**連接詞時，句子還接不接得起來」。
同一層、方向互補。

⏳ **實例待補** —— 下次實際抓到時填進來。⛔ 不編一個。
...
剩下的句子若都能獨立存在，這段是在**列舉**而非**論述**，接第 8 問處理。
本模組的問句是那個自查的展開版：不只問「有沒有連接」，而是問**連接的兩邊站不站在同一個軸上**。
...
⛔ **段間承接不歸本模組**：由 section 模組定義、在 `structure-first.md` 的承接欄實例化。
本模組只管段內，⛔ 不要再往這裡加段間問句。

⚠️ 唯一看起來跨段的是第 6 問，但它抓的是**同一個歸因講了兩次／換了說法**，不是承接斷裂。兩者不同。
