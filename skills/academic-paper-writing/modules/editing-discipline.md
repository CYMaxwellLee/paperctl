...
| 層級 | 手段 | 誰動手 | 說明 |
|------|------|--------|------|
| **A. Rei 可自行執行** | **方陣魔法（換短同義字）** | Rei | 看每段最後一行，把該段前面的字換成**同義的短字**，讓那一行被吸上去。⭐ 主人 2026-07-29：「改字其實就會吸上去，根本不會動到意思，同樣意思的字，有長有短」「這個還蠻固定的招式」「這些會讓空間利用率大幅度提升，**就不一定要用上 vspace**」。見下方兩條界線 |
| | 調整圖片尺寸／環境 | Rei | `width=0.85\linewidth`；或 `figure*`（跨欄）↔ `figure`（單欄）互換。★ 2026-07-29 實證：Figs. 1--3 全部單欄化一次省下**整頁**，是當天唯一真正見效的排版手段 |
...
### ⭐ 方陣魔法的兩條界線（⛔ 兩條都要守，2026-07-29 主人訂）

**① 換短同義字 ≠ 刪減內容。** 方陣換的是**表達的長度**，claim、evidence、機制解釋、limitation 一個都沒少，所以它屬於 A 層，**不受本節「刪減內容需同意」的限制**。⛔ 別拿最高鐵則來擋方陣 —— 那是兩件事。真正被禁的是**拿掉一段承載了東西的文字**。

**② ⛔ 方陣只屬於排版階段，不得下滲到寫作階段。**

> 我們一開始寫作的時候，不用上方陣魔法，這個是最後排版才用的，因此**要記住寫作時還是正常正式嚴謹寫作，不要為了方陣而綁手綁腳**（主人 2026-07-29）

| 階段 | 做法 |
|------|------|
| **寫作／改寫** | 正常、正式、嚴謹地寫。⛔ **不考慮長度**，不為了之後好收而先寫短、先挑短字 |
| **最後排版** | 才開始方陣：找 orphan → 量「需省幾字元」→ 換短同義字 → 重編驗證 |

⚠️ 這條跟本節開頭「⛔ 不准在寫作當下因為怕超頁而把該寫的分析先寫短」是**同一條規則的兩半**：一個禁「先縮內容」，一個禁「先縮用詞」。**兩半一起才擋得住。**

### 方陣的操作要領

- **目標是段落的最後一行**：只剩一兩個字（orphan）的那種。量它的寬度 → 換算成「這段前面需要省下幾個字元」。
- **⛔ 不要靠機械替換**（自動同義詞表）。換字會動到 register，而 register 比禁詞表更嚴（見 `style-guide.md`）。工具只負責**指出哪裡、要省多少**，換哪個字是判斷。
- **⛔ 不要靠 `\looseness=-1`** —— 已實證無效，見上。
- **改完一定重編驗證**：斷行預測不可靠，「這段真的少一行了嗎」可靠。主人 2026-07-29 的原話是「**一頁一頁 recursive 截圖 修理 orphan word 修好，不要走捷徑**」。
- **⛔ 排除 references／caption／表格內的短行**：bibliography 格式固定，而 references 頁通常不佔正文頁限。不排除就是一堆雜訊。
