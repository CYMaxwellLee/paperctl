...
> 新裁決先進帳本，再視情況編進模組。
> **記錄與使用格式（2026-07-06 教授：「不要死背…你的任務是把大架構架設起來，
> 不是建立字典給底層背」）**：每條裁決寫 (a) 教授原話、(b) 它揭示的**底層目標**、
> (c) 具體句子或修法——標為**例證**。從糾正學習的方式是更新目標函數，不是擴充
> 字典；**例證不得升格為規則或句式清單**。Proxy 對照本帳本時，比對的是目標
> 有沒有達成，不是字面有沒有出現。
...
- **One-shot 檢討批次**（2026-07-06，含教授同日修正）：「檢討我們這幾輪對話…
  讓未來我們都很容易 one shot 完成」→ (1) **句式看功能不背字典**——¶4 開頭三輪
  後被接受的是「To this end, we propose」，但教授隨即修正：「不一定 always 要
  這樣寫…也可以 Enlightened by 這類…不要死背」「我在意的是專業、流暢、把重點
  痛點講清楚」→ 底層目標＝連接功能＋專業流暢；例句只是例證，禁句式清單；
  (2) **全文等強審查**——「sentences 之間的 transition 做得更流暢我會覺得更好」；
  **沒糾正≠核可**（糾正是最痛問題的抽樣）；(3) **Director 狀態外部化**——每篇
  論文維護 `PAPER-STATE.md`，狀態不准只活在對話 context（過期數字事故根源）；
  (4) **pipeline light profile**——修訂類任務走 Section Editor→Voice→Verifier→
  Proxy 短路徑。
