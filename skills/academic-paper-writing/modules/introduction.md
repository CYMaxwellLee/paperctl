...
剩空殼（「一個機制」「一組實驗」）就重寫。
**We 開頭沒問題**（教授 2026-07-06 明示：「Contributions 可以 We 開頭沒關係」）——
這條規則從來不是禁字，是禁「內容空洞」：句法上避開了 We、內容仍是機制清單或
數字報告，等於沒改。驗收永遠用結果測試：reviewer 只讀這三條 bullets，他知道
這篇為什麼重要嗎？
**每條要短**（教授 2026-07-06：「每一項都有點太長了」）：一條 bullet 一個主張，
一到兩句收掉；enumeration 與鋪陳能刪就刪，數字保留（文采原則，非字數配額）。
