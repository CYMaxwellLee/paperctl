...
const EXEMPLARS = ROOT + '/skills/academic-paper-writing/modules/approved-exemplars.md'
const DOCTRINE = [STYLE_GUIDE, LEDGER, EXEMPLARS, ...(A.moduleFiles || []).map(f => f.startsWith('/') ? f : ROOT + '/' + f)]
...
const GLOBAL_RULES = '\nGLOBAL RULES (the doctrine outranks this edit brief — if they conflict, follow the doctrine and flag the conflict):\n' +
  ...
  '- WRITE FOR FLOW FIRST (教授 2026-07-06: transitions must read 一氣呵成, never as independent short sentences): draft linearly in ONE continuous pass so each sentence grows out of the previous one, matching the cadence of the approved exemplars. Do NOT satisfy checkable constraints mid-sentence at the cost of flow — downstream gates and your own revision passes will catch mechanical violations; fixing a flagged word later is cheap, fragmented prose is not.\n' +
  '- CHAIN READ (mandatory; the professor\'s most-repeated correction): read every consecutive sentence PAIR, including the pair spanning the previous paragraph\'s last sentence. Each pair needs an explicit link — a connective phrase, a subject that picks up the previous sentence\'s content (given-new), or an unambiguous anaphor. Connective phrases are ENCOURAGED and the adverb-opener ban does not apply to them (To this end, / To do so, / Under this regime, / In this way, / Beyond X, are EXAMPLES, not an inventory — any construction that performs the connective function fluently is acceptable; never memorize phrases, judge by function). If a sentence could be removed or reordered unnoticed, the chain is broken. Do not insert stand-alone bridge sentences; let the continuing sentence carry the connection itself.\n' +
...
const proxyPrompt = (text) => readDoctrine + CONTEXT +
  '\nYou are the PROFESSOR-PROXY REVIEWER. The rulings ledger (among the doctrine files above) is your ground truth. Review the candidate exactly as the professor would, line by line: register line-edits (ledger §一), argument/structure/clarity (§二 — transitions, over-compressed sentences, referent alignment, cross-paragraph redundancy, selling vs narrative, ownership marker), claim safety (§三), conventions (§四). Cite the ledger entry in rulingRef for every finding. Verdict approve ONLY if the professor would plausibly accept without line edits. Professor-approved conventions are binding — do not flag them. Review the WHOLE text at equal rigor — mid-paragraph sentence-to-sentence flow matters as much as paragraph boundaries (the professor explicitly wants smoother transitions everywhere; his past corrections are a sample of the worst problems, NOT an exhaustive list, so the absence of a past correction is never approval). Judge structural moves by FUNCTION and fluency, never by whether they use any particular stock phrase — enforcing a phrase inventory is forbidden (2026-07-06 ruling: 不要死背). When unsure whether something violates a ruling, raise it in overall as an open question instead of inventing a finding.\n\nCANDIDATE:\n' + text
...
while (rounds < 2 && (!verifier.pass || proxy.verdict === 'revise')) {
  ...
  const revision = await agent(
    readDoctrine + CONTEXT +
    '\nYou are the JUDGE (Voice) revising your own final text. Apply the verifier issues and professor-proxy line edits below. Doctrine and ledger outrank everything; do not regress any global check (ownership, selling, redundancy, terminology). Revise by REWRITING the text in one continuous pass that incorporates every accepted finding — never by splicing patched sentences into the old text; after rewriting, chain-read every consecutive pair. Return the COMPLETE revised text, not a diff.\n\nCURRENT TEXT:\n' + judgement.final +
